...
def generate_daily_executive_pdf(
    ...
    """Generate the redesigned customer-facing Daily Executive Report."""

    from app.services.daily_executive_pdf_service import (
        generate_daily_executive_pdf as build_daily_executive_pdf,
    )

    return build_daily_executive_pdf(
        live_kpis,
        language=language,
    )
...
def generate_weekly_operations_pdf(
    ...
    """Generate the redesigned customer-facing Weekly Operations Report."""

    from app.services.weekly_operations_pdf_service import (
        generate_weekly_operations_pdf as build_weekly_operations_pdf,
    )

    return build_weekly_operations_pdf(
        weekly_kpis,
        language=language,
    )


def _generate_weekly_operations_pdf_legacy(
    weekly_kpis: dict,
):
    """
...
def generate_monthly_kpi_pdf(
    ...
    """Generate the Monthly KPI Pack using the isolated UX V2 renderer."""
    from app.services.monthly_kpi_pdf_service import (
        generate_monthly_kpi_pdf as build_monthly_kpi_pdf,
    )

    return build_monthly_kpi_pdf(
        monthly_kpis,
        language=language,
    )


def _generate_monthly_kpi_pdf_legacy(
    monthly_kpis: dict,
):
    """
