...
def _normalize_language(
    ...
    """
    Normalize language values used by executive insights.
...
def _find_recommendation(
    ...
    If the trend engine does not return a recommendation,
    use a language-aware fallback.
    """
    ...
    if normalized_language == "mn":
        if severity in {"critical", "high"}:
            return (
                f"{kpi_name} үзүүлэлтийн гүйцэтгэлд "
                "үйл ажиллагааны нарийвчилсан хяналт хийж, "
                "хариуцах эзнийг тодорхойлно уу."
            )

        if severity == "medium":
            return (
                f"{kpi_name} үзүүлэлтэд нөлөөлж буй "
                "гол хязгаарлалтуудыг шалгаж, дараагийн "
                "тайлант хугацаанд үргэлжлүүлэн хянана уу."
            )

        return (
            "Үйл ажиллагааны одоогийн хэмнэлийг хадгалж, "
            "тэргүүлэх үзүүлэлтүүдийн хяналтыг "
            "үргэлжлүүлнэ үү."
        )
